...
export default class DatePicker extends Component {
	...
	*/
	passDateToParent() {
		if (typeof this.props.dateReceiver === 'function') this.props.dateReceiver(this.state.currentDate);
	...
	render({ opened }) {
		...
		return (
				...
				<div class={"datePicker " + (opened && "datePicker--opened")} >
					<div class="datePicker--titles">
						<h3 style={{
							...
							}} onClick={this.toggleYearSelector}>{currentDate.getFullYear()}
						</h3>
						<h2 style={{
							color: !selectYearMode ? 'rgba(255,255,255,.87)' : 'rgba(255,255,255,.57)'
			...
			<div class="datePicker--background" onClick={this.closeDatePicker} style={{display: opened ? 'block' : 'none'}}/>
			</div>
		)
	}
}
